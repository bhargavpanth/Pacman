...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        # print state[0]
        # print state[1]
        # the position of the dots - [(1, 1), (1, 6), (6, 1), (6, 6)]
        # each of the four corners of the maze
        # state[1][0] - (1, 1), state[1][1] - (1, 6), state[1][2] - (6, 1), state[1][3] - (6, 6)
        # co_ordinates = state[0]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            nextx = int(x + dx)
            nexty = int(y + dy)
            # if no hitting the walls
            if not self.walls[nextx][nexty]:
                cost = 1
                # new position is nextx, nexty
                new_position = (nextx,nexty)
                if (new_position == self.corners[0]):
                    self.corner_one = True
                if (new_position == self.corners[1]):
                    self.corner_two = True
                if (new_position == self.corners[2]):
                    self.corner_three = True
                if (new_position == self.corners[3]):
                    self.corner_four = True
                nextState = (new_position,(self.corner_one,self.corner_two,self.corner_three,self.corner_four))
                successors.append( ( nextState, action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    while (len(unvisited_corners) > 0):
        ...
        distance = distance + closest_corner[0]
...
def foodHeuristic(state, problem):
    ...
    for current_food in food:
        for select_food in food:
    ...
    if(max_distance[0]==(0,0) and max_distance[1]==(0,0)):
        heuristic = util.manhattanDistance(position,food[0])
    else: 
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # A-Start
        actions = search.aStarSearch(problem, 0)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        x,y = state
        grid = self.food
        if (grid[x][y] == True) or (grid.count() == 0):
            return True
